...
#define GPIO3_REG_MUX_EN1     0x2C  // EN for GPIOs 3.2, 3.3, 3.4
#define GPIO3_MUX_EN_B1_1     0x1F  // in AND

#define GPIO3_REG_EN          0x30
#define GPIO3_REG_DIR         0xF0
#define GPIO3_REG_BUFFER      0xF1
#define GPIO3_REG_INV_LOGIC   0xF2
#define GPIO3_REG_STATUS      0xE7
...
#define GPIO3_MASK_EN          0x02

#define GPIO3_EN          0x01
#define GPIO3_DIR_IN      0x01
#define GPIO3_INV_LOGIC   0x01
...
#define WBGPIO_INFO(fmt, arg...) printk(KERN_INFO "W83627DHG_GPIO3: " fmt "\n" , ## arg)
...
static int w83627dhg_gpio_get_value (struct gpio_chip *gc, unsigned offset) {
	...
	ret = chip->superio_read (GPIO3_REG_BUFFER, &reg_value);
	if (ret < 0) {
...
static void w83627dhg_gpio_set_value (struct gpio_chip *gc, unsigned offset, int value) {
	...
	ret = chip->superio_write (GPIO3_REG_BUFFER, reg_out);
	if (ret < 0) {
		pr_err ("%s, operation failed!\n", __func__);
		goto out;
	}
...
static ssize_t gpio_directions_show (struct device *dev,
	...
	ret = pdata->superio_read (GPIO3_REG_DIR, &reg);
	if (ret < 0) {
...
static ssize_t gpio_values_show (struct device *dev,
	...
	int ret;
	pdata = dev->platform_data;
	ret = pdata->superio_read (GPIO3_REG_BUFFER, &reg);
	if (ret < 0) {
...
static int w83627dhg_gpio_direction_input (struct gpio_chip *gc, unsigned offset) {
	...
	chip->dir_output &= ~(GPIO3_DIR_IN << offset);
	chip->dir_input |= GPIO3_DIR_IN << offset;

	reg_value = seco_gpio_exp_direction_reg (chip);

	chip->superio_write (GPIO3_REG_DIR, (int)reg_value);
...
static int seco_w83627dgh_gpio_setup (struct platform_device *device, struct seco_w83627dhg_gpio_platform_data *pdata, struct w83627dhg_gpio_chip *chip) {
	...
	/* set and init the gpio hardware registers */
		// set the pin muxing
        chip->superio_read (GPIO3_REG_MUX_EN1, &reg);
	reg &= GPIO3_MUX_EN_B1_1;
        chip->superio_write (GPIO3_REG_MUX_EN1, reg);	

		// enable GPIO3
	chip->superio_read (GPIO3_REG_EN, &reg);
	chip->superio_write (GPIO3_REG_EN, reg | GPIO3_MASK_EN);
		// set all GPIO's direction
	chip->superio_write (GPIO3_REG_DIR, 0xFF & seco_gpio_exp_direction_reg(chip));
		// set all GPIOs as not inverted
	chip->superio_write (GPIO3_REG_INV_LOGIC, (~(0xFF * GPIO3_INV_LOGIC)) & GPIO_ALL);
...
static int __devinit seco_w83627dhg_gpio_probe (struct platform_device *pdev) {
	...
	WBGPIO_INFO ("SECO W83627 GPIO3 Expander star probe");
	...
	WBGPIO_INFO ("W83627 GPIO3 Expander probed!");
