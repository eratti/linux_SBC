...
static struct event_dev event_list [] = {
...
static inline int getFlag (struct i2c_client *client, int event) {
	...
	return (reg & event_list[event].index_reg) >> event_list[event].shift;
}
...
static inline void setFlag (struct i2c_client *client, int event, int set) {
	int reg = (int)getFlagReg(client);
	reg = (set) ? reg | (1u << event_list[event].shift) : reg & ~(1u << event_list[event].shift);
...
static int ectrl_proc_state_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_state_show, PDE(inode)->data);
}
...
static int ectrl_proc_status_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_status_show, PDE(inode)->data);
}
...
static int ectrl_proc_enable_write (struct file *file, const char __user *buf,
	...
	int err_conv;
	long en;
	struct ectrl_proc_event *proc = (struct ectrl_proc_event *)PDE(file->f_path.dentry->d_inode)->data;
	if (!proc)
		return -EINVAL;
...
static int ectrl_proc_enable_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_enable_show, PDE(inode)->data);
}
...
static int ectrl_proc_enflash_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_enflash_show, PDE(inode)->data);
}
...
static int ectrl_proc_pm_pwr_state_write (struct file *file, const char __user *buf,
	...
	int err_conv;
	long en;
	struct econtroller *ectrl = (struct econtroller *)PDE(file->f_path.dentry->d_inode)->data;
	if (!ectrl)
		return -EINVAL;
...
static int ectrl_proc_pm_pwr_state_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_pm_pwr_state_show, PDE(inode)->data);
}
...
static int ectrl_proc_available_pm_state_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_available_pm_state_show, PDE(inode)->data);
}
...
static int ectrl_proc_bootdev_write (struct file *file, const char __user *buf,
	...
	int err_conv;
	long  id;
	struct ectrl_proc_boot *proc = (struct ectrl_proc_boot *)PDE(file->f_path.dentry->d_inode)->data;
	if (!proc)
		return -EINVAL;
...
static int ectrl_proc_bootdev_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_bootdev_show, PDE(inode)->data);
}
...
static int ectrl_proc_bootdev_list_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_bootdev_list_show, PDE(inode)->data);
}
...
static int ectrl_proc_bootseq_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_bootseq_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_enable_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_enable_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_event_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_event_show, PDE(inode)->data);
}
...
static int ectrl_proc_available_wdt_event_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_avalaible_wdt_event_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_delay_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_delay_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_timeout_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_timeout_show, PDE(inode)->data);}
...
static int ectrl_proc_wdt_refresh_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_refresh_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_restore_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_restore_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_wdtout_reset_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_wdtout_reset_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_timer1_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_timer1_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_timer2_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_timer2_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_state_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_state_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_f_enable_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_f_enable_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_f_event_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_f_event_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_f_delay_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_f_delay_show, PDE(inode)->data);
}
...
static int ectrl_proc_wdt_f_timeout_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_f_timeout_show, PDE(inode)->data);
}
		...
		switch (ectrl->events[i]->id) {
			case PWR_BUTTON:
				...
				entry = proc_create_data(ECTRL_ENTRY_ENABLE,  S_IRUGO | S_IWUGO,
				 		event_list[ectrl->events[i]->id].proc_dir, &ectrl_proc_enable_fops, 
						pwr_btn_eproc);
				if (!entry) {
                                        goto remove_dir;		
			        } else {
					add_to_list (tmp, entry_list, ECTRL_ENTRY_ENABLE, event_list[ectrl->events[i]->id].proc_dir);
				}
			
				entry = proc_create_data(ECTRL_ENTRY_ENFLASH,  S_IRUGO | S_IWUGO,
				 		event_list[ectrl->events[i]->id].proc_dir, &ectrl_proc_enflash_fops, 
						pwr_btn_eproc);
				if (!entry) {
                                        goto remove_dir;
			        } else {
					 add_to_list (tmp, entry_list, ECTRL_ENTRY_ENFLASH, event_list[ectrl->events[i]->id].proc_dir);
				}

#if ECTRL_USE_OWN_STATE
				entry = proc_create_data(ECTRL_ENTRY_STATUS,  S_IRUGO,
				 		event_list[ectrl->events[i]->id].proc_dir, &ectrl_proc_state_fops, 
						pwr_btn_eproc);
				if (!entry) {
                                        goto remove_dir;
			        } else {
					add_to_list (tmp, entry_list, ECTRL_ENTRY_STATUS, event_list[ectrl->events[i]->id].proc_dir);
				}
#endif
				break;
...
static void manage_event (struct econtroller *ectrl, struct event_dev *event) {
	switch (event->id) {
		case PWR_BUTTON:
			...
			input_report_key (event->input, KEY_POWER, 1);
			input_sync (event->input);
			input_report_key (event->input, KEY_POWER, 0);
			input_sync (event->input);
			break;
		case RST_BUTTON:
		case SLEEP_SIGNAL:
		...
		case BATLOW_HL_SIGNAL:
			printk (KERN_INFO "event name: %s\n", event->name);
			input_report_key (event->input, KEY_BATTERY, 1);
			input_sync (event->input);
			input_report_key (event->input, KEY_BATTERY, 0);
			input_sync (event->input);
			break;
		case BATLOW_LH_SIGNAL:
			printk (KERN_INFO "event name: %s\n", event->name);
...
static void ectrl_resolve_state (struct econtroller *ectrl, int enable, int flags, int status) {
	...
	for (i = 0 ; i < nr_events ; i++) {
		if (GET_EVENT_FLAG(flags, ectrl->events[i]) == 1) {
			manage_event (ectrl, ectrl->events[i]);
			flags &= ~(1 << event_list[ectrl->events[i]->id].shift);
		}
	}
	// we have to clean the served event's flag 
...
static int __devinit ectrl_probe(struct i2c_client *client, const struct i2c_device_id *id) {

	int err, ret, regs = 0;
	int retval = 0, idx, nre, i, idxs, bidx;
	int enable_reg, flag_reg, status_reg;
	struct ectrl_platform_data *pdata = (struct ectrl_platform_data *)client->dev.platform_data;
	struct event_dev **events;
	if (!pdata) {
	...
	nre = 0;
	for (i = 0 ; i < pdata->nr_event ; i++) {
		if (pdata->event_list[i] == EVNT_BATTERY || pdata->event_list[i] == EVNT_LID)
			nre += 2;
		if (pdata->event_list[i] == EVNT_PWR_BTN || pdata->event_list[i] == EVNT_RST_BTN ||
				pdata->event_list[i] == EVNT_SLEEP || pdata->event_list[i] == EVNT_WAKE)
			nre++;
	}
	
	ectrl->nr_evn = board_nr_states[bidx]; 
	ectrl->evn_state = kzalloc (sizeof (struct event_state) * ectrl->nr_evn, GFP_KERNEL);
	if (!ectrl->evn_state) {
		err = -ENOMEM;
		goto err_evn_state;
	}
	events = kzalloc (sizeof (struct econtroller) * nre, GFP_KERNEL);
	if (!events) {
		err = -ENOMEM;
		goto err_events;
	}

	// N.B. the pwr_btn_4sec is not a true event, so it will be filtered.
	for (idx = 0, idxs = 0, i = 0 ; i < pdata->nr_event ; i++) {
		if (board_reg_state_idx[bidx][pdata->event_list[i]] != -1) {
			ectrl->evn_state[idxs] = &event_state_list[pdata->event_list[i]];
			ectrl->evn_state[idxs]->reg_idx = board_reg_state_idx[bidx][pdata->event_list[i]];
			idxs++;
		}
		switch (pdata->event_list[i]) {
			case EVNT_PWR_BTN:
				events[idx] = &event_list[PWR_BUTTON];
				idx++;
				break;
			case EVNT_RST_BTN:
				events[idx] = &event_list[RST_BUTTON];
				idx++;
				break;
			case EVNT_SLEEP:
				events[idx] = &event_list[SLEEP_SIGNAL];
				idx++;
				break;
			case EVNT_BATTERY:
				events[idx] = &event_list[BATLOW_HL_SIGNAL];
				events[idx + 1] = &event_list[BATLOW_LH_SIGNAL];
				idx += 2;
				break;
			case EVNT_LID:
				events[idx] = &event_list[LID_HL_SIGNAL];
				events[idx + 1] = &event_list[LID_LH_SIGNAL];
				idx += 2;
				break;
			case EVNT_WAKE:
				events[idx] = &event_list[WAKE_SIGNAL];
				idx ++;
				break;
			default:
				break;
	...
	ectrl->nr_bootdev   = pdata->nr_bootdev;
	ectrl->bootdev_list = kzalloc (sizeof (enum BOOTDEV_ID) * ectrl->nr_bootdev, GFP_KERNEL);
	if (!ectrl->bootdev_list) {
		err = -ENOMEM;
		goto err_bootdev_list;
	}

	for (i = 0 ; i < ectrl->nr_bootdev ; i++) {
		ectrl->bootdev_list[i].id = (enum BOOTDEV_ID)pdata->available_bootdev[i].id;
		ectrl->bootdev_list[i].label = kzalloc (sizeof (char) * 
					strlen (pdata->available_bootdev[i].label),
					GFP_KERNEL);
		strcpy (ectrl->bootdev_list[i].label, pdata->available_bootdev[i].label);
	}
	...
	ectrl->irq          = pdata->irq;
	ectrl->irq_flags    = pdata->irq_flags;
	...
	ectrl->phys = kzalloc (sizeof (char) * ectrl->nr_evnt, GFP_KERNEL);
	for (i = 0 ; i < ectrl->nr_evnt ; i++) {
	
		ectrl->events[i]->input = input_allocate_device();
		if (!ectrl->events[i]->input) {
        	        err = -ENOMEM;
                	goto err_input;
	        }

		ectrl->phys[i] = kzalloc (sizeof (char) * 32, GFP_KERNEL);
		snprintf(ectrl->phys[i], sizeof(char) * 32,
				"%s/input%d", dev_name(&client->dev), i);

		ectrl->events[i]->input->name = ectrl->events[i]->input_name;
		ectrl->events[i]->input->phys = ectrl->phys[i];
		ectrl->events[i]->input->id.bustype = BUS_I2C;
		ectrl->events[i]->input->dev.parent = &client->dev;

		ectrl->events[i]->input->evbit[0] = BIT_MASK(EV_KEY);

		if (ectrl->events[i]->id == PWR_BUTTON) 
			set_bit (KEY_POWER, ectrl->events[i]->input->keybit);
		else if (ectrl->events[i]->id == BATLOW_HL_SIGNAL || ectrl->events[i]->id == BATLOW_LH_SIGNAL)
			set_bit (KEY_BATTERY, ectrl->events[i]->input->keybit);
		else
			set_bit (KEY_RESERVED, ectrl->events[i]->input->keybit);

		err = input_register_device (ectrl->events[i]->input);
		if (err) {
	...
	ret = request_irq (pdata->irq, irq_interrupt_manager,
				pdata->irq_flags, "e_controller", &ectrl);
	...
	retval = misc_register(&ectrl_device);
...
err_reg_input:
	for (i = 0 ; i < regs ; i++) {
		input_unregister_device (ectrl->events[i]->input);
	}
err_input:
	for (i = 0 ; i <= regs; i++) {
		kfree (ectrl->events[i]->input);
	}
err_bootdev_list:
	ectrl->nr_bootdev = 0;
err_evn_state:
...
static int __devexit ectrl_remove(struct i2c_client *client) {
...
static struct i2c_driver ectrl_driver = {
	...
	.probe		= ectrl_probe,
	.remove		= __devexit_p(ectrl_remove),
