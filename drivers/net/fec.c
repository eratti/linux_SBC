...
#if defined(CONFIG_MICREL_KSZ9031)
static struct workqueue_struct *ksz_autoneg;

typedef struct {
  struct work_struct	 ksz_autoneg_work;
  struct phy_device	*ksz_dev;
//  int 	maxfaultcount;
  int 	onetimecheck;
  int 	onetimecheck_lock;
} ksz_autoneg_t;
...
#if defined(CONFIG_MICREL_KSZ9031)
static void ksz_autoneg_function( struct work_struct *work)
{
int val, pairX, x, i, ig;
    	int tmp;
    	int max = 5000;
    	int phy_event = 0x0;
    	ksz_autoneg_t *ksz_work = (ksz_autoneg_t *) work;
	if (!ksz_work->ksz_dev) {
		ksz_work->onetimecheck_lock = 0;
		return;
	}
	struct phy_device *kszdev = ksz_work->ksz_dev;
	phy_event = phy_read(kszdev, 0x1B);
	
	//if (phy_event == 0x28) {  // micrel is negotiating a link.
		phy_write(kszdev, MII_BMCR, 0x8000); // phy reset
	    udelay(10); // Wait for reset to complete
	    phy_write(kszdev, MII_NCONFIG, 0x00C0); // disable Auto MDI/MDI-X
   	    phy_write(kszdev, 0x0, 0x8000); // phy reset
    	udelay(10); // wait for reset to complete
   	    ksz_work->onetimecheck = 1;
    	kszdev->supported &= PHY_BASIC_FEATURES;
	   	/* enable phy pause frame for any platform */
	   	kszdev->supported |= ADVERTISED_Pause;
    	kszdev->advertising = kszdev->supported;

   	    phy_write(kszdev, MII_NCONFIG, 0x0000); // enable Auto MDI/MDI-X
	
    	tmp = phy_read(kszdev, MII_CTRL1000);
    	tmp &= 0xFCFF;
    	phy_write(kszdev, MII_CTRL1000, tmp);
    	tmp = phy_read(kszdev, MII_BMCR);
    	tmp |= BMCR_ANRESTART;  // 0x0200;
    	phy_write(kszdev, MII_BMCR, tmp);
   	    ksz_work->onetimecheck = 1;
    	ksz_work->onetimecheck_lock = 0;
//	}
}
...
#if defined(CONFIG_MICREL_KSZ9031)
	struct phy_device *kszdev = fep->phy_dev;
    	ksz_autoneg_t *ksz_autoneg_work = (ksz_autoneg_t *)work;
    	if ((ksz_autoneg_work->onetimecheck == 0) & (ksz_autoneg_work->onetimecheck_lock == 0)) {
	  ksz_autoneg_work->onetimecheck_lock = 1;
	  ksz_autoneg_work->ksz_dev = kszdev;
    	  queue_work( ksz_autoneg, (struct work_struct *)work );
    	}
#endif
...
#if defined(CONFIG_MICREL_KSZ9031)
	ksz_autoneg = create_workqueue("ksz_autoneg");
	if (ksz_autoneg) {
	  /* Queue some work (item 1) */
	  work = (ksz_autoneg_t *)kmalloc(sizeof(ksz_autoneg_t), GFP_KERNEL);
	  if (work) {
	    INIT_WORK( (struct work_struct *)work, ksz_autoneg_function );
	    work->ksz_dev = NULL;
//	    work->maxfaultcount = 0;
	    work->onetimecheck = 0;
	    work->onetimecheck_lock = 0;
	  }
	}
#endif
